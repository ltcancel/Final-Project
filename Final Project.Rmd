...
#select subset  
happy2019 <- select(happy2019,Region,Country,Happiness.Rank,Happiness.Score,Health..Life.Expectancy.,Generosity,year)
head(happy2019)

happy2018 <- select(happy2018,Region,Country,Happiness.Rank,Happiness.Score,Health..Life.Expectancy.,Generosity,year)
happy2017 <- select(happy2017,Region,Country,Happiness.Rank,Happiness.Score,Health..Life.Expectancy.,Generosity,year)
happy2016 <- select(happy2016,Region,Country,Happiness.Rank,Happiness.Score,Health..Life.Expectancy.,Generosity,year)
happy2015 <- select(happy2015,Region,Country,Happiness.Rank,Happiness.Score,Health..Life.Expectancy.,Generosity,year)

master.happy <- rbind(happy2015,happy2016,happy2017,happy2018,happy2019)
master.happy
```
...
```{r}
group.master <- master.happy %>%
  group_by(Region,year) %>%
  summarise(avg_score = mean(Happiness.Score))

ggplot(group.master,aes(avg_score)) + geom_area(stat = "bin")

qplot(year,data = group.master,geom = "histogram",fill = avg_score)
```
